...
# -- A plot is made for each CpG. Here the datasets are loaded and unloaded for each CpG.
#    This could be made faster (but more memory-intensive) effectively switching the loops.
for (horvath_cpg in horvath_ids) {
    ...
    for (data_id in horvath_450k_sets) {
        ...
        horvath_Pos <- dat$Pos[dat$Probe == horvath_cpg]
        window_size = 10000
        dat <- dat %>%
    ...
    # -- Removing data to prepare for next CpG
    rm(full_dat, tmp, myd)
