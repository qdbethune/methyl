...
# -- Terminate if output files already exist
...
for (data_id in dataset_list) {
    ...
    if (startsWith(data_id, "GSE")) {
        ...
        if (grepl("day", age_type)) {
            tmp_age$Age <- tmp_age$Age / 365
            
        } else if (grepl("week", age_type)) {
            tmp_age$Age <- tmp_age$Age * (7 / 365)
            
        } else if (grepl("month", age_type)) {
            tmp_age$Age <- tmp_age$Age / 12
        } 
    } 
...
rm(setG, beta, pheno, pheno_data, age_type, data_id, sampleNames, sampleList, tmp_met, tmp_age)
